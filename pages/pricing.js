  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '48px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
          {plans.map(plan => (
            <div key={plan.id} style={{
              ...
              <button
                onClick={() => plan.id === 'free' ? router.push(session ? '/' : '/auth') : subscribe(plan.id)}
        ...
        {checkoutError && (
          <div style={{ marginTop: 20, padding: '12px 16px', background: '#2a0a0a', border: '1px solid #6b1a1a', borderRadius: 4, fontSize: 14, color: '#f87171', textAlign: 'center' }}>
            {checkoutError}
          </div>
        )}
